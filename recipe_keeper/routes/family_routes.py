...
@family_bp.route('/families/<family_id>/members', methods=['POST'])
@login_required
def add_family_member(family_id):
    ...
    try:
        ...
        if not user_result:
            ...
            conn.commit()
			invite_link = f"localhost:3000/invite/accept/{invitation_token}"
			msg = Message(
			subject="You're invited to Recipe Keeper",
			sender="noreply@yourdomain.com",
			recipients=[email],
			body=f"""
			Hi {first_name}, You've been invited to join the Recipe Keeper family group by {current_user.email}.Click the link below to accept the invite and access shared recipes:{invite_link}If you did not expect this, you can ignore this message. Thanks,Recipe Keeper Team""")
			mail.send(msg)
...
def create_family():
    ...
    try:
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (family_id, current_user.id, 'admin', True))
...
@family_bp.route('/families/<family_id>/members', methods=['GET'])
@login_required
def get_family_members(family_id):
    ...
    try:
        ...
        family_id_str = str(uuid.UUID(family_id))
...
@family_bp.route('/families/<family_id>/members/<member_id>', methods=['DELETE'])
@login_required
...
@family_bp.route('/recipes/<recipe_id>', methods=['DELETE'])
@login_required
...
@family_bp.route('/invite/accept/<token>', methods=['GET'])
def accept_invite(token):
    conn = get_db_connection()
    cur = conn.cursor()

    cur.execute("""
        SELECT id, email FROM family_members
        WHERE invitation_token = %s AND invitation_status = 'pending'
    """, (token,))
    record = cur.fetchone()
    ...
    # Now associate this member with the currently logged-in user
    if not current_user or not current_user.is_authenticated:
        return jsonify({"status": "error", "message": "Login required to accept invite"}), 401

    family_member_id = record["id"]
    invited_email = record["email"]

    # Optional: check that current_user.email == invited_email
    if current_user.email != invited_email:
        return jsonify({"status": "error", "message": "Email mismatch"}), 403

    cur.execute("""
        UPDATE family_members
        SET user_id = %s, invitation_status = 'accepted'
        WHERE id = %s
    """, (current_user.id, family_member_id))

    conn.commit()
    return jsonify({"status": "success", "message": "Invitation accepted"}), 200
